...
**As built (4.6)**, in `packages/agents/src/sideboard.ts`: `sideboard(input)` is a pure function of data — the main deck and sideboard as `DeckSlot`s, what each card is (tags from `@mtg/cards`, land or not, the colours it costs and makes; `@mtg/sim`'s `sideboardCardsFor` builds this from definitions), what the opponent has been seen to play, the matchup record and each card's `MatchupRecord`, and the ban list — and returns a `SideboardPlan`: the swaps, each with the score and basis of both cards, and the two lists after. A card with a record in this matchup scores its win rate when drawn less its win rate when not, each shrunk toward the deck's matchup win rate by twenty games (docs/05); a card without one scores by its tags — `priorWeight` × (the share of the opponent's seen nonland cards its `vs` tags answer − ½); a card with neither has no score and is never swapped on a guess, so early in a run the prior swaps answer for answer. Swaps pair the best sideboard card with the worst main card while the difference beats `margin`, up to `maxSideboardSwaps` (a run setting, default 4): land for land and spell for spell, never a spell whose colours the lands will not make, never a land whose removal would strand a spell, never a banned card. The plan is the record docs/04 asks for; the match runner (5.1) applies it between games 2 and 3 and stores it with the match.
