...
- Sanity ladders: `search` beats `greedy` beats `random` with statistically significant margins over 500 games (checked with a binomial test; guards against an evaluator regression that makes the AI worse). The first rung, `greedy` over `random`, runs in the unit suite as 60 games on fuzz boards with alternating seats and an exact one-sided binomial tail below 0.001 over the decided games (`packages/sim/src/game.test.ts`); the full 500-game ladder arrives with the `search` level.
