...
- **A ban asked for while the run plays** reaches it through `DriveOptions.banRequests`, which the run drains after every game and as every cycle starts, so the edit takes effect after the game in progress (5.7: the worker pool hands the operator's edits to the run's worker this way).
