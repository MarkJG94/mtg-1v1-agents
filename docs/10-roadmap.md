...
- [~] 1.9 Continuous effects and the layer system with timestamps and dependency. *(Layers 1-7 with all five of layer 7's sublayers, timestamp order within a layer, durations including until-end-of-turn and while-source-on-battlefield, and per-state memoisation. Every characteristic reader in the engine now goes through `characteristics(state, id)` — the stored values on `GameObject` are printed values only. **Dependency (CR 613.8) is outstanding**: effects apply in timestamp order, which is right except where one changes what another applies to (Humility + Opalescence, Blood Moon + Urza lands). Static abilities move here from 1.8 and are simply continuous effects with `whileSourceOnBattlefield` duration.)*
