...
describe('a game from two decks', () => {
  it('puts each main deck in its owner’s library, and leaves the sideboards out', async () => {
  ...
  it('refuses a deck holding a card nobody has scripted', async () => {
    const unknown = {
      ...deck,
...
describe('the match', () => {
  it('stops when a player has won two games, and never plays a fourth', async () => {
    for (const seed of ['m1', 'm2', 'm3', 'm4', 'm5', 'm6']) {
      const result = await match({ seed });
  ...
  it('lets the first chooser choose game 1, and the loser of each game choose the next', async () => {
    for (const [seed, firstChooser] of [
      ...
      const result = await match({ seed, firstChooser });
   ...
   */
  it('keeps the same chooser after a drawn game, and calls level wins a drawn match', async () => {
    const result = await match({ turnCap: 2, firstChooser: 'B' });
  ...
  it('puts on the play whoever the chooser chooses', async () => {
    const plays = await match({
  ...
  it('is the same match from the same seed', async () => {
    expect(await match({ seed: 'again' })).toEqual(await match({ seed: 'again' }));
  });
});

/** A seed whose match runs to exactly this many games, found by playing. */
const seedFor = async (games: number): Promise<string> => {
  for (let i = 0; i < 30; i += 1) {
    const seed = `length-${games}-${i}`;
    if ((await match({ seed })).games.length === games) return seed;
  }
  throw new Error(`no seed in thirty gives a ${games}-game match`);
};

describe('sideboarding between games 2 and 3 (docs/04)', async () => {
  /** A match that goes to three games, so the hook has a game 3. */
  const threeGames = await seedFor(3);
  ...
  it('starts from the legalised deck when a ban took effect after game 2 (docs/05)', async () => {
    // The triggers are "banned" after game 2, and four more creatures take their place.
    const legal: Deck = {
      main: [slot(fuzzLand, 22), slot(fuzzCreature, 20), slot(fuzzBurn, 14), slot(fuzzRemoval, 4)],
      side: deck.side,
    };
    const shown: Deck[] = [];
    const result = await match({
      ...
      players: {
        A: {
          ...
          agent: greedyAgent(),
          sideboard: (context) => {
            shown.push(context.deck);
            return { swaps: [], main: context.deck.main, side: context.deck.side };
          },
        },
        B: { deck, agent: greedyAgent() },
      },
      afterGame: async ({ gameId, decks }) =>
        gameId.endsWith(':game-2')
          ? { decks: { A: legal, B: decks.B }, definitions: new Map(), changes: { A: [], B: [] } }
          : null,
    });
    expect(shown).toEqual([legal]);
    expect(result.games[2]?.decks.A).toEqual(legal.main);
    expect(result.decks.A).toEqual(legal);
    expect(result.legalisations.map((l) => l.afterGameId)).toEqual([`${threeGames}:game-2`]);
  });

  it('asks once, before game 3, and plays game 3 with what it answered', async () => {
  ...
  it('shows the hook what the opponent has shown, and only that', async () => {
  ...
  it('is not asked in a match that ends in two games', async () => {
    const twoGames = await seedFor(2);
    let asked = 0;
    await match({
      seed: twoGames,
      players: {
  ...
  it('refuses a plan that changes the seventy-five or the sixty', async () => {
    ...
    for (const plan of [extra, short]) {
      ...
      ).rejects.toThrow(IllegalSideboardError);
    }
  });
});
